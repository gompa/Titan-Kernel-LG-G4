		...
		if (ctrl_pdata->ndx) {
			...
			case PROXY_NEAR:
				if (ctrl_pdata->panel_data.panel_info.lge_pan_info.panel_type
					...
					if (ctrl_pdata->lge_pan_data->dsv_manufacturer == DSV_SM5107){
						sm5107_fast_discharge();
					}
					else if (ctrl_pdata->lge_pan_data->dsv_manufacturer == DSV_DW8768){
...
|| defined(CONFIG_MACH_MSM8992_P1_GLOBAL_COM) || defined(CONFIG_MACH_MSM8992_SJR_GLOBAL_COM)
...
int lgd_qhd_command_mdss_dsi_event_handler(struct mdss_panel_data *pdata, int event, void *arg)
{
	...
	case MDSS_EVENT_PANEL_OFF:
		if (ctrl_pdata->ndx) {
			if (ctrl_pdata->lge_pan_data->dsv_manufacturer == DSV_SM5107){
				sm5107_lgd_dsv_setting(0);    /* LPWG mode */
			}
			else if (ctrl_pdata->lge_pan_data->dsv_manufacturer == DSV_DW8768){
